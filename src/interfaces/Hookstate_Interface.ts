...
export default function useHookstateGetters() {
  ...
  const getSettingsReady = () =>
    sceneState.sceneSettings.ready.get({ noproxy: true });
  const getMusic = () => sceneState.sceneSettings.music.get({ noproxy: true });
  const getSFX = () => sceneState.sceneSettings.sfx.get({ noproxy: true });
  const getAnnouncer = () =>
    sceneState.sceneSettings.announcer.get({ noproxy: true });
  const getAllSoundsMuted = () =>
    sceneState.sceneSettings.muteAllSound.get({ noproxy: true });
  const getGameID = () => sceneState._id.get({ noproxy: true });
  const getRoomCode = () => sceneState.roomCode.get({ noproxy: true });
  const setGameID = (id: string) => sceneState._id.set(id);
  const setRoomCode = (code: number) => sceneState.roomCode.set(code);

  const setGameConfigs = (
    avatar: AvatarSelectionType,
