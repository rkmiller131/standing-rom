...
export class PoseSolver {
  ...
  static solve(
    ...
    if (video) {
      const videoEl = (
        typeof video === 'string' ? document.querySelector(video) : video
      ) as HTMLVideoElement;
    ...
    const rightHandOffscreen =
      lm3d[15].y > 0.1 ||
      (lm3d[15].visibility ?? 0) < 0.23 ||
      0.995 < lm2d[15].y;
    const leftHandOffscreen =
      lm3d[16].y > 0.1 ||
      (lm3d[16].visibility ?? 0) < 0.23 ||
      0.995 < lm2d[16].y;

    const leftFootOffscreen =
      lm3d[23].y > 0.1 ||
      (lm3d[23].visibility ?? 0) < 0.63 ||
      Hips.Hips.position.z > -0.4;
    const rightFootOffscreen =
      lm3d[24].y > 0.1 ||
      (lm3d[24].visibility ?? 0) < 0.63 ||
      Hips.Hips.position.z > -0.4;
...
export class HandSolver {
  ...
  static solve(
    ...
    handRotation.y =
      side === LEFT ? handRotation.z - 0.3 : handRotation.z * PI + 0.3;
    ...
    segments.forEach((j) => {
      ...
      if (e === 'Thumb') {
        ...
        if (j === 'Proximal') {
          ...
          newThumb.x = clamp(
            startPos.x + trackedFinger.z * -PI * dampener.x,
            -0.6,
            0.3,
          );
          newThumb.y = clamp(
