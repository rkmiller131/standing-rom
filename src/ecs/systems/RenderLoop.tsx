...
interface RenderLoopProps {
  ...
  renderBubbles: (arr: EntityUUID[]) => void;
}

export default function RenderLoop({ avatar, renderBubbles }: RenderLoopProps) {
  const { engine, execute, createEntity } = useEngine();
  const clock = useRef(engine.clock);
  clock.current.reset();
  const gameState = useGameState();
  const sceneState = useSceneState();

  useEffect(() => {
    // This execute loops for half the frames (so if you have 60FPS, this code block executes 30FPS)
    execute(() => {
    ...
    let gameEnded = false;
    // make a new property on gameState for sets and reps, just for easy access around the app
    const numTargets = 5;
    execute(() => {
      if (gameEnded) return;
      ...
      if (levels && levels.length > 0) {
        // console.log('levels at 0 is ', levels[0])
        if (levels[0].inPlay === false) {
          ...
          for (let i = 0; i < numTargets; i++) {
            ...
            gameState.levels[0].bubbleEntities[i].set(bubbleEntity.entityUUID as EntityUUID);
          }

          const bubbleEntities = gameState.levels[0].bubbleEntities.get({noproxy: true});
          const filteredBubbleEntities = bubbleEntities.filter((entity): entity is EntityUUID => entity!== null && entity!== undefined);
          renderBubbles(filteredBubbleEntities.slice());
  ...
  // EDIT: Can't do this, it TANKS performance
  // return all the physical representations of the game logic so this will be a body of our entities
  // map over the gamestate levels, and spawn that many bubble entities.
  // each bubble entity will have its own collection of components, managed either by the useEffect in gamelogic or on the bubble entity itself.
  // return (
  //   <>
  //     <Bubble position={[0.7, 1.2, 0.1]} />
  //     <Bubble position={[0.8, 1.6, 0.3]} />
  //     <Bubble position={[0.6, 1.4, 0.3]} />
  //   </>
  // );
  return null;
}
